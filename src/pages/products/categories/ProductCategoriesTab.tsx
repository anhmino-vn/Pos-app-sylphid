...
export function ProductCategoriesTab({ globalSearch = '' }: { globalSearch?: string }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap: any) => {
      ...
      setCategories(data);
      setLoading(false);
    }, (err: any) => { handleFirestoreError(err, OperationType.LIST, 'productCategories'); setLoading(false); });
    return () => unsub();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, 'productCategories', editingId), { ...payload, updatedAt: serverTimestamp() });
        toast.success('Cập nhật danh mục thành công');
      } else {
        await addDoc(collection(db, 'productCategories'), { ...payload, sortOrder: categories.length, createdAt: serverTimestamp() });
        toast.success('Thêm danh mục thành công');
      }
      setIsModalOpen(false);
    } catch (err) { handleFirestoreError(err, OperationType.WRITE, 'productCategories'); }
    finally { setIsSubmitting(false); }
  };

  const handleStatusToggle = async (node: CategoryNode) => {
    const newStatus = node.status === 'active' ? 'inactive' : 'active';
    await updateDoc(doc(db, 'productCategories', node.id!), { status: newStatus, updatedAt: serverTimestamp() });
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    await Promise.all(reordered.map((id, idx) =>
      updateDoc(doc(db, 'productCategories', id), { sortOrder: idx, updatedAt: serverTimestamp() })
    ));
    setActiveId(null);
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h2 className="text-base font-black text-slate-800">Cây danh mục sản phẩm</h2>
