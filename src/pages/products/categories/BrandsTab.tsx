...
import { Plus, Edit2, Trash2, Tag, X, Loader2, RefreshCw } from 'lucide-react';
...
export function BrandsTab() {
  ...
  const [formData, setFormData] = useState<Partial<Brand>>({ name: '', description: '', logoUrl: '', website: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot: any) => {
      ...
      snapshot.forEach((doc: any) => {
        data.push({ id: doc.id, ...doc.data() } as Brand);
      });
      setBrands(data);
      setLoading(false);
    }, (error: any) => {
  ...
  const handleOpenModal = (brand?: Brand) => {
    if (brand) {
      ...
      setFormData({ name: brand.name, description: brand.description || '', logoUrl: brand.logoUrl || '', website: brand.website || '' });
    } else {
      setEditingId(null);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-black text-slate-800">Thương hiệu</h2>
          <p className="text-sm text-slate-500">Quản lý các thương hiệu sản phẩm</p>
        </div>
        <button
              ...
              brands.map(brand => (
                <tr key={brand.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-6 py-4">
                    ...
                    {brand.website && <div className="text-xs text-blue-500 hover:underline"><a href={brand.website} target="_blank" rel="noopener noreferrer">{brand.website}</a></div>}
                  </td>
                  <td className="px-6 py-4 text-slate-500 max-w-md truncate">{brand.description || '-'}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenModal(brand)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => setDeleteId(brand.id!)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors">
            ...
            >
              <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{editingId ? 'Sửa thương hiệu' : 'Thêm thương hiệu'}</h2>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Tên thương hiệu <span className="text-rose-500">*</span></label>
                  <input 
                    required
                    autoFocus
                    type="text"
      ...
      <ConfirmModal
        ...
        message="Bạn có chắc chắn muốn xóa thương hiệu này? Hành động này không thể hoàn tác."
