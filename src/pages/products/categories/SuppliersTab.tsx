...
import { Plus, Edit2, Trash2, Truck, X, Loader2, RefreshCw } from 'lucide-react';
...
export function SuppliersTab() {
  ...
  const [formData, setFormData] = useState<Partial<Supplier>>({ 
    name: '', phone: '', email: '', address: '', contactPerson: '', debtBalance: 0 
  });
  ...
  const handleOpenModal = (supplier?: Supplier) => {
    if (supplier) {
      ...
      setEditingId(null);
      setFormData({ name: '', phone: '', email: '', address: '', contactPerson: '', debtBalance: 0 });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-black text-slate-800">Nhà cung cấp</h2>
          <p className="text-sm text-slate-500">Quản lý đối tác và công nợ</p>
        </div>
        <button
            ...
            {suppliers.length === 0 ? (
              ...
              suppliers.map(supplier => (
                <tr key={supplier.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-slate-600 font-medium">{supplier.contactPerson || '-'}</td>
                  <td className="px-6 py-4 text-right font-bold text-rose-600">
                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(supplier.debtBalance || 0)}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenModal(supplier)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => setDeleteId(supplier.id!)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors">
            ...
            >
              <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{editingId ? 'Sửa nhà cung cấp' : 'Thêm nhà cung cấp'}</h2>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Tên nhà cung cấp <span className="text-rose-500">*</span></label>
                    <input 
                      required
                      autoFocus
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Người liên hệ</label>
                    <input 
                      type="text"
                      value={formData.contactPerson}
                      onChange={e => setFormData({ ...formData, contactPerson: e.target.value })}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 outline-none font-medium text-slate-700 transition-all"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Điện thoại</label>
                    <input 
                      type="tel"
      ...
      <ConfirmModal
        ...
        message="Bạn có chắc chắn muốn xóa nhà cung cấp này? Nếu họ đang có công nợ hoặc giao dịch, bạn không nên xóa."
